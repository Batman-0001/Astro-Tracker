...
import { calculateRiskScore, getRiskLevelInfo } from '../services/riskEngine.js';
...
router.post('/fetch', auth, async (req, res, next) => {
    try {
        const { type = 'today' } = req.body;

        console.log(`🔄 Manual fetch triggered by ${req.user.email} (type: ${type})`);
...
// @access  Private
router.get('/test-nasa', auth, async (req, res, next) => {
    try {
...
// @access  Private
router.get('/test-risk/:id', auth, async (req, res, next) => {
    try {
