...
const Dashboard = () => {
    ...
    const filteredAsteroids = asteroids.filter((asteroid) => {
        if (searchQuery && !asteroid.name.toLowerCase().includes(searchQuery.toLowerCase())) {
            return false;
        }
        if (filter === 'hazardous' && !asteroid.isPotentiallyHazardous) return false;
        if (filter === 'high' && asteroid.riskCategory !== 'high') return false;
        if (filter === 'moderate' && asteroid.riskCategory !== 'moderate') return false;
        if (filter === 'low' && asteroid.riskCategory !== 'low') return false;
        return true;
    });

    const closestApproach = todayAsteroids.length > 0
        ? todayAsteroids.reduce((prev, curr) =>
            (curr.missDistanceLunar || Infinity) < (prev.missDistanceLunar || Infinity) ? curr : prev
        )
        : null;
    ...
    return (
        <div className="min-h-screen pt-20 pb-10">
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 items-center">
                        ...
                        >
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-accent-primary font-medium">Real-time Tracking</span>
                            </div>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <StatCard
                            icon="⚠️"
                            label="Hazardous"
                            value={stats?.hazardous || todayAsteroids.filter(a => a.isPotentiallyHazardous).length}
                        ...
                        <StatCard
                            ...
                            value={
                                todayAsteroids.length > 0
                                    ? Math.round(
                                        todayAsteroids.reduce((sum, a) => sum + (a.riskScore || 0), 0) / todayAsteroids.length
                                    )
                                    : '—'
                            }
                            subValue="Risk score"
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    {isLoading ? (
                        ...
                        <div className="glass p-12 text-center">
                            ...
                            <p className="text-white/50">No asteroids found matching your criteria</p>
                        </div>
                    )}
            ...
            {/* High Risk Section */}
            {todayAsteroids.filter(a => a.riskCategory === 'high' || a.isPotentiallyHazardous).length > 0 && (
                <section className="px-6 py-8">
                    <div className="max-w-7xl mx-auto">
                        <motion.div
                        ...
                        >
                            ...
                            <p className="text-white/50 mb-4">
                                These asteroids require close monitoring due to their size, velocity, or proximity.
                            </p>
                            <div className="grid gap-2">
                                {todayAsteroids
                                    .filter(a => a.riskCategory === 'high' || a.isPotentiallyHazardous)
                                    .slice(0, 5)
                                    .map((asteroid, index) => (
                                        <AsteroidCard
