...
import {
    ArrowLeft, Eye, EyeOff, AlertTriangle, Gauge, ExternalLink,
    Calendar, Ruler, Zap, Target, Globe, Share2, Bookmark
} from 'lucide-react';
...
const AsteroidDetail = () => {
    ...
    const { selectedAsteroid, fetchAsteroidById, isLoading, addToWatchlist, removeFromWatchlist, isInWatchlist } = useAsteroidStore();
    ...
    const getSizeComparison = (diameterM) => {
        ...
        if (diameterM < 10) return { name: 'Car', icon: '🚗', scale: diameterM / 5 };
        if (diameterM < 50) return { name: 'Blue Whale', icon: '🐋', scale: diameterM / 30 };
        if (diameterM < 100) return { name: 'Statue of Liberty', icon: '🗽', scale: diameterM / 93 };
        if (diameterM < 200) return { name: 'Football Field', icon: '🏈', scale: diameterM / 110 };
        if (diameterM < 450) return { name: 'Empire State Building', icon: '🏢', scale: diameterM / 443 };
        ...
        <div className="min-h-screen pt-20 pb-10">
            ...
            <section className="relative px-6 py-12 overflow-hidden">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className={`absolute top-0 right-0 w-96 h-96 rounded-full blur-[150px] pointer-events-none ${asteroid.riskCategory === 'high' ? 'bg-risk-high/20' :
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className={`px-3 py-1 rounded-full text-sm font-bold ${getRiskBadgeClass(asteroid.riskCategory)}`}>
                        ...
                        <div className="flex gap-3">
                            {isAuthenticated && (
                                <motion.button
                                    ...
                                    className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-all ${isWatched
                            ...
                            <a
                                href={asteroid.nasaJplUrl || `https://ssd.jpl.nasa.gov/tools/sbdb_lookup.html#/?sstr=${asteroid.neo_reference_id}`}
                                target="_blank"
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        >
                            ...
                            <p className="text-white/40 text-sm">
                                {Math.round(asteroid.relativeVelocityKmH || 0).toLocaleString()} km/h
                            </p>
                        </motion.div>
                        ...
                        >
                            ...
                            <p className="text-white/40 text-sm">
                                {Math.round(asteroid.estimatedDiameterMin || 0)}m - {Math.round(asteroid.estimatedDiameterMax || 0)}m
                            </p>
                        </motion.div>
                        ...
                        >
                            ...
                            <div className="mt-8 space-y-4">
                                <div className="flex items-center gap-4">
                                    <span className="w-32 text-right text-white/50">{sizeComparison.name}</span>
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="w-32 text-right text-white/50">Asteroid</span>
                                    <div className="flex-1 bg-space-700 rounded-full h-4">
                                        <motion.div
                                            className="bg-gradient-to-r from-accent-primary to-accent-secondary h-full rounded-full"
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min(100, sizeComparison.scale * 30)}%` }}
                            ...
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between text-sm mb-1">
                                        ...
                                        <span className={asteroid.isPotentiallyHazardous ? 'text-risk-high' : 'text-risk-minimal'}>
                                    ...
                                    <div className="h-2 bg-space-700 rounded-full">
                                        <div
                                            ...
                                            style={{ width: asteroid.isPotentiallyHazardous ? '100%' : '10%' }}
                                        />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between text-sm mb-1">
                                        ...
                                        <span className="text-white">{Math.round(asteroid.estimatedDiameterMax || 0)}m</span>
                                    </div>
                                    <div className="h-2 bg-space-700 rounded-full">
                                        <div
                                            className="h-full rounded-full bg-accent-secondary"
                                            style={{ width: `${Math.min(100, (asteroid.estimatedDiameterMax || 0) / 10)}%` }}
                                        />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between text-sm mb-1">
                                        ...
                                        <span className="text-white">{asteroid.missDistanceLunar?.toFixed(2)} LD</span>
                                    </div>
                                    <div className="h-2 bg-space-700 rounded-full">
                                        <div
                                            className="h-full rounded-full bg-risk-moderate"
                                            style={{ width: `${Math.max(5, 100 - (asteroid.missDistanceLunar || 50) * 2)}%` }}
                                        />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between text-sm mb-1">
                                        ...
                                        <span className="text-white">{asteroid.relativeVelocityKmS?.toFixed(1)} km/s</span>
                                    </div>
                                    <div className="h-2 bg-space-700 rounded-full">
                                        <div
                                            className="h-full rounded-full bg-risk-low"
                                            style={{ width: `${Math.min(100, (asteroid.relativeVelocityKmS || 0) * 5)}%` }}
                                        />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="relative">
                                    <svg className="w-48 h-48 transform -rotate-90">
                                        ...
                                        <motion.circle
                                            ...
                                            cy="96"
                                            r="88"
                                            className={`${asteroid.riskCategory === 'high' ? 'stroke-risk-high' :
                                                    asteroid.riskCategory === 'moderate' ? 'stroke-risk-moderate' :
                                                        asteroid.riskCategory === 'low' ? 'stroke-risk-low' : 'stroke-risk-minimal'
                                                }`}
                                            strokeWidth="12"
                                            fill="none"
                                            strokeLinecap="round"
                                            ...
                                            animate={{ strokeDashoffset: 553 - (asteroid.riskScore / 100) * 553 }}
                                            transition={{ duration: 1.5, delay: 0.3 }}
                                        />
                                    </svg>
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-5xl font-bold text-white">{asteroid.riskScore}</span>
                                        <span className={`text-sm font-medium capitalize ${asteroid.riskCategory === 'high' ? 'text-risk-high' :
                                                asteroid.riskCategory === 'moderate' ? 'text-risk-moderate' :
                                                    asteroid.riskCategory === 'low' ? 'text-risk-low' : 'text-risk-minimal'
                                            }`}>
            ...
            <section className="px-6 py-8">
                <div className="max-w-6xl mx-auto">
                    <h2 className="text-2xl font-bold text-white mb-6">Orbital Information</h2>
                    <motion.div
                        className="glass p-6"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
