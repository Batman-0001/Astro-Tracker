...
const AsteroidCard = ({ asteroid, index = 0, compact = false }) => {
    const { addToWatchlist, removeFromWatchlist, isInWatchlist } = useAsteroidStore();
    ...
    if (compact) {
        return (
            <Link to={`/asteroid/${asteroid.neo_reference_id}`}>
                ...
                >
                    {/* Risk indicator */}
                    <div className={`w-2 h-12 rounded-full ${asteroid.riskCategory === 'high' ? 'bg-risk-high' :
                    ...
                    <div className={`px-2 py-1 rounded-lg text-xs font-bold ${getRiskBadgeClass(asteroid.riskCategory)}`}>
                        {asteroid.riskScore}
                    </div>
            ...
            >
                ...
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${getRiskBadgeClass(asteroid.riskCategory)}`}>
